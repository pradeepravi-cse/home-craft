...
import { Plus, Package, Edit2, Trash2, AlertTriangle, RotateCcw } from 'lucide-react'
...
export default function ProductsPage() {
  ...
  const [pForm, setPForm] = useState({
    name: '', description: '', category: 'COOKIES', price: '',
    costPrice: '', unit: 'per piece', isActive: true, isPublic: true,
  })
  const [txForm, setTxForm] = useState({ type: 'in', quantity: '', notes: '', unitCost: '' })

  const loadProducts = () => productsApi.list(categoryFilter || undefined).then(setProducts)
  ...
  const openEdit = (p: any) => {
    ...
    setPForm({
      ...
      unit: p.unit || 'per piece', isActive: p.isActive, isPublic: p.isPublic,
    })
    setModal(true)
  }

  const saveProduct = async () => {
    if (!pForm.name || !pForm.price) { toast.error('Name and price are required'); return }
    try {
      const data = {
        ...pForm,
        price: parseFloat(pForm.price),
        costPrice: parseFloat(pForm.costPrice) || undefined,
      }
  ...
  const saveTransaction = async () => {
    try {
      await inventoryApi.addTransaction({
        productId: txModal.id, type: txForm.type,
        quantity: parseFloat(txForm.quantity),
        notes: txForm.notes || undefined,
        unitCost: parseFloat(txForm.unitCost) || undefined,
      })
  ...
  return (
    <div className="pb-6">
      <PageHeader
        title="Products"
        subtitle="Bakery catalog & inventory"
        action={
      ...
      {tab === 'products' && (
        <>
          ...
          <div className="px-4 mb-3 flex gap-2 overflow-x-auto pb-1">
            {[['', 'All'], ...Object.entries(CATEGORY_LABELS)].map(([v, l]) => (
              <button key={v} onClick={() => setCategoryFilter(v)}
                className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${categoryFilter === v ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700'}`}>
                {l}
              </button>
            ))}
          </div>
          ...
          {products.length === 0 ? (
            <Empty icon={<Package size={40} />} message="No products yet"
              action={<button onClick={openNew} className="btn-primary text-sm">Add first product</button>} />
          ) : (
                  ...
                  <div key={p.id} className="card">
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">{CATEGORY_LABELS[p.category] || p.category} {p.unit ? `· ${p.unit}` : ''}</p>
                      ...
                      <div className="flex flex-col items-end gap-1 ml-3 flex-shrink-0">
                        ...
                        <div className="flex gap-1.5 mt-1">
                          <button onClick={() => openEdit(p)} className="p-1.5 text-gray-500 hover:text-white bg-gray-800 rounded-lg transition-colors"><Edit2 size={12} /></button>
                          <button onClick={() => setDeleteId(p.id)} className="p-1.5 text-gray-500 hover:text-red-400 bg-gray-800 rounded-lg transition-colors"><Trash2 size={12} /></button>
                          <button onClick={() => setTxModal(p)} className="p-1.5 text-gray-500 hover:text-brand-400 bg-gray-800 rounded-lg transition-colors" title="Update stock"><Package size={12} /></button>
                        </div>
                      </div>
                    </div>
                    {s && (
      ...
      {tab === 'inventory' && (
        <div className="px-4 space-y-2">
          {stock.length === 0 ? (
            <Empty icon={<Package size={40} />} message="No stock records. Add transactions from Products tab." />
          ) : (
            stock.map(s => {
      ...
      <Modal open={modal} onClose={() => { setModal(false); setEditProduct(null) }} title={editProduct ? 'Edit Product' : 'New Product'}>
        <div className="space-y-3">
          <Field label="Name *">
            <input className="input" value={pForm.name} onChange={e => setPForm(f => ({ ...f, name: e.target.value }))} placeholder="Product name" />
          </Field>
          <Field label="Description">
            <textarea className="input" value={pForm.description} onChange={e => setPForm(f => ({ ...f, description: e.target.value }))} placeholder="Optional description" />
          </Field>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Selling Price (RM) *">
              <input type="number" step="0.01" className="input" value={pForm.price} onChange={e => setPForm(f => ({ ...f, price: e.target.value }))} />
            </Field>
            <Field label="Cost Price (RM)">
              <input type="number" step="0.01" className="input" value={pForm.costPrice} onChange={e => setPForm(f => ({ ...f, costPrice: e.target.value }))} />
            </Field>
          </div>
          <Field label="Unit">
            <input className="input" value={pForm.unit} onChange={e => setPForm(f => ({ ...f, unit: e.target.value }))} placeholder="e.g. per piece, per dozen" />
          </Field>
      ...
      <Modal open={Boolean(txModal)} onClose={() => setTxModal(null)} title={`Update Stock: ${txModal?.name}`}>
        <div className="space-y-3">
          <div className="flex gap-2">
            {[['in', '📦 Stock In'], ['out', '📤 Stock Out'], ['adjustment', '⚖️ Adjust']].map(([v, l]) => (
          ...
          <Field label={txForm.type === 'adjustment' ? 'Set Stock To' : 'Quantity'}>
            <input type="number" step="0.1" className="input" value={txForm.quantity} onChange={e => setTxForm(f => ({ ...f, quantity: e.target.value }))} />
          </Field>
          {txForm.type === 'in' && (
            <Field label="Unit Cost (RM)">
              <input type="number" step="0.01" className="input" value={txForm.unitCost} onChange={e => setTxForm(f => ({ ...f, unitCost: e.target.value }))} />
            </Field>
          )}
          <Field label="Notes">
            <input className="input" value={txForm.notes} onChange={e => setTxForm(f => ({ ...f, notes: e.target.value }))} placeholder="Optional notes" />
          </Field>
